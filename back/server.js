...
app.get("/auth/logout", (req, res) => {
  ...
  return res.status(202).clearCookie("jwt").json({ Msg: "cookie cleared" })
    .send;
});
...
app.post("/posts", async (req, res) => {
  try {
    ...
    const name = await pool.query("SELECT username FROM users WHERE id = $1", [
      user_id,
    ]);
    const username = name.rows[0].username;

    const newpost = await pool.query(
      "INSERT INTO posts(body, user_id, created_time, comment_count) values ($1, $2, $3, $4) RETURNING*",
      [body, user_id, formattedCreatedTime, comment_count]
    );
...
app.get("/posts/:id", async (req, res) => {
  try {
    ...
    if (!posts.rows[0]) {
      return res
        .status(404)
        .json({ err: `This post with id ${id} not found.` });
    }
...
app.put("/posts/:id", async (req, res) => {
  try {
    ...
    if (like_count) {
      updatepost = await pool.query(
        "UPDATE posts SET like_count = $2 WHERE id = $1 RETURNING*",
        [id, like_count]
      );
    }
    console.log("updatedPost", updatepost);
    const name = await pool.query("SELECT username FROM users WHERE id = $1", [
      updatepost.rows[0].user_id,
    ]);
    const username = name.rows[0].username;
...
app.delete("/posts/:id", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: `This post with id ${id} successfully deleted.` });
  } catch (err) {
    console.error(err.message);
    return res.status(500).json({ err: err.message });
  }
